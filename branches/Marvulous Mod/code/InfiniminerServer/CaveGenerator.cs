    ...
    public class CaveGenerator
    {
        ...
        public static BlockType[, ,] GenerateCaveSystem(int size, bool includeLava)
        {
            ...
            if (SessionVariables.sandboxMode == false)
            {
                ...
                for (int i = 0; i < SessionVariables.oreFactor; i++)
        ...
        public static void AddLava(ref BlockType[, ,] data, int size)
        {
            ...
            if (InfiniminerServer.lavaAtGroundLevel == 2)
            {
        ...
        public static void AddGrass(ref BlockType[, ,] data, int size)
        {
            for (ushort x = 0; x < size; ++x)
            {
                for (ushort y = 0; y < size; ++y)
                {
                    if (data[x, y, GlobalVariables.GROUND_LEVEL + 1] == BlockType.Dirt && data[x, y, GlobalVariables.GROUND_LEVEL] == BlockType.None)
                    {
                        data[x, y, GlobalVariables.GROUND_LEVEL + 1] = BlockType.DirtGrass;
                    }
                }
            }
        }
